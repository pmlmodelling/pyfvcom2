...
# Import version information. ``version.py`` is generated by setup.py, so a
# source checkout can legitimately be missing it before the package is built.
try:
...
from .restart import interpolate_restart_data, write_restart
...
# Import tide prediction and management. UTide is only required for tide
# prediction, so keep the rest of the package importable when it is absent.
try:
    from .tide import TideManager, predict_tide
except ModuleNotFoundError as exc:
    if exc.name is None or not exc.name.startswith("utide"):
        raise
    _utide_missing_message = (
        "UTide is required for pyfvcom2 tide prediction tools. "
        "Install the 'utide' package to use {}."
    )

    def predict_tide(*args, **kwargs):
        raise ModuleNotFoundError(_utide_missing_message.format("predict_tide"))

    class TideManager:
        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError(_utide_missing_message.format("TideManager"))
