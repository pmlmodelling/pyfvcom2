...
def write_restart(
    ...
    with Dataset(template_file_path, "r") as template_ds, Dataset(
        ...
        if new_datetime is not None:
            ...
            output_ds.variables["time"][:] = np.asarray(
                new_time, dtype=template_ds.variables["time"].datatype
            )
            output_ds.variables["Itime"][:] = np.asarray(
                new_Itime, dtype=template_ds.variables["Itime"].datatype
            )
            output_ds.variables["Times"][:] = np.asarray(
                new_times, dtype=template_ds.variables["Times"].datatype
            )
